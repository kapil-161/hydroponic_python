...
echo "[1/2] Running Treatment T01: CO2:400_EC:1.5_Light:16_Nitrogen:200_RootZoneTemp:20_Temperature:23_Varieties:LET_EXP001_2024_pH:6.0"
echo "Creating treatment-specific input files..."
mkdir -p temp_input_T01
cp -r ../input/* temp_input_T01/
echo "Setting CO2 to 400 ppm for treatment T01..."
sed -i '' 's/co2_concentration,.*/co2_concentration,400/' temp_input_T01/LET_EXP001_2024_system_settings.csv
...
echo "[2/2] Running Treatment T02: CO2:1200_EC:1.5_Light:16_Nitrogen:200_RootZoneTemp:20_Temperature:23_Varieties:LET_EXP001_2024_pH:6.0"
echo "Creating treatment-specific input files..."
...
echo "Setting root zone temperature to 20°C for treatment T02..."
sed -i '' 's/optimal_temperature,.*/optimal_temperature,20/' temp_input_T02/LET_EXP001_2024_root_zone_parameters.csv
...
if [ -f temp_treatments/treatment_T01.csv ]; then
    tail -n +2 temp_treatments/treatment_T01.csv | while IFS=, read -r date day treatment_id system_id crop_id rest; do
        echo "$date,$day,T01_CO2400_EC1.5_LIGHT16_NITROGEN200_ROOTZONETEMP20_TEMPERATURE23_VARIETIESLET_PH6.0,$system_id,$crop_id,$rest" >> ../outputs/LET_EXP001_2024_combined_results.csv
    done
fi
if [ -f temp_treatments/treatment_T02.csv ]; then
    tail -n +2 temp_treatments/treatment_T02.csv | while IFS=, read -r date day treatment_id system_id crop_id rest; do
        echo "$date,$day,T02_CO21200_EC1.5_LIGHT16_NITROGEN200_ROOTZONETEMP20_TEMPERATURE23_VARIETIESLET_PH6.0,$system_id,$crop_id,$rest" >> ../outputs/LET_EXP001_2024_combined_results.csv
    done
fi
