#!/usr/bin/env python3
"""
Comprehensive Range Analysis of NFT Lettuce Simulation Outputs
...
def analyze_key_metrics(df):
    ...
    print("="*80)
    print("NFT LETTUCE SIMULATION - COMPREHENSIVE RANGE ANALYSIS")
    print("="*80)
    ...
    key_metrics = {
        ...
        'WUE_kg_m3': {'name': 'Water Use Efficiency', 'unit': 'kg/m³', 'expected_range': (10, 100)},
        ...
        'EC': {'name': 'Electrical Conductivity', 'unit': 'mS/cm', 'expected_range': (1.0, 5.0)},
...
def analyze_harvest_metrics(df):
    ...
    standards = {
        ...
        'Water use efficiency': {
            'simulated': final_day['WUE_kg_m3'] if 'WUE_kg_m3' in df.columns else 0,
...
def identify_issues(df):
    ...
    if 'WUE_kg_m3' in df.columns:
        wue_values = df['WUE_kg_m3'].dropna()
        if len(wue_values) > 0:
            max_wue = wue_values.max()
            min_wue = wue_values.min()
            if max_wue > 200:
                issues.append(f"Unrealistically high WUE: {max_wue:.1f} kg/m³")
            if min_wue < 1:
                issues.append(f"Unrealistically low WUE: {min_wue:.1f} kg/m³")
...
def analyze_environmental_factors(df):
    ...
    env_factors = {
        ...
        'EC': {'name': 'EC', 'unit': 'mS/cm', 'optimal_range': (1.5, 2.5)}
    }
...
def generate_summary_report():
    ...
    # Load data for final assessment
    ...
    if 'WUE_kg_m3' in df.columns:
        final_wue = final_values['WUE_kg_m3']
        if final_wue >= 20:
